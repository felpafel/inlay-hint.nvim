...
function M.on_inlayhint(err, result, ctx, _)
    ...
    local function pos_to_byte(position)
        ...
        if col > 0 then
            ...
            ok, convert_result = pcall(
                util._str_byteindex_enc,
                line,
                col,
...
function M.get(filter)
    ...
    if not bufnr then
        ...
        vim.tbl_map(function(buf)
            vim.list_extend(
                hints,
                M.get(vim.tbl_extend('keep', { bufnr = buf }, filter))
            )
                ...
                for _, hint in pairs(hints) do
                    ...
                    if
                        (
                            ...
                            or char >= range.start.character
                        )
                        and (
                            line < range['end'].line
                            or char <= range['end'].character
                        )
                    then
                        table.insert(result, {
                            bufnr = bufnr,
...
function M.setup(opts)
    ...
    for _, client in
        pairs(vim.lsp.get_clients({ method = 'textDocument/inlayHint' }))
    do
